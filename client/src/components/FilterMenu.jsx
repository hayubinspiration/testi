...
const FilterMenu = forwardRef(({ setData, isVisible, setVisibility }, ref) => {
  ...
  const filterBtns = [
    ...
    {
      name: "Price",
      values: ["₹0-₹1000", "₹1000-₹2000", "₹2000-₹3000", "₹3000+"],
    },
    { name: "Category", values: ["Men", "Women", "Kids", "Unisex"] },
    {
      name: "Rating",
      values: ["1⭐ & Up", "2⭐ & Up", "3⭐ & Up", "4⭐ & Up"],
    },
    {
      name: "Discount",
      values: [
  ...
  const handleFilter = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/products/filterBy?${queryString}`,
      );
              ...
              {activeFilter === index && (
                <div className="xs:text-sm md:text-base">
                  {btn.values.map((value, id) => (
                    <div
                      key={id}
                      className="flex justify-between hover:bg-gray-900 hover:text-white w-full border-b-1"
                    >
                      <button
                        className="px-4 md:py-1 "
                        onClick={() =>
                          handleFilterChange(btn.name.toLowerCase(), value)
                        }
                      >
                        {value}
                      </button>
